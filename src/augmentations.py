    ...
    for data_dir in utils.load_config('datasets'):
        if os.path.exists(data_dir):
            ...
            if not os.path.exists(fp):
                print(
                    f'Warning: path {fp} does not exist, falling back to {data_dir}'
                )
                fp = data_dir
...
def random_overlay(x, dataset='places365_standard'):
    ...
    if dataset == 'places365_standard':
        ...
        imgs = _get_places_batch(batch_size=x.size(0)).repeat(
            1,
            x.size(1) // 3, 1, 1)
    else:
...
def prepare_pad_batch(obs, next_obs, action, batch_size=32):
    ...
    batch_next_obs = batch_from_obs(
        torch.from_numpy(next_obs).cuda(), batch_size)
    batch_action = torch.from_numpy(action).cuda().unsqueeze(0).repeat(
        batch_size, 1)
...
def cutout_color(x, size=32):
    """
    Inserts a small rectangular patch of random color into the observation at a random position.

    x: (B, C, H, W), size: int
    """
    B, C, H, W = x.shape

    top_left_y = torch.randint(0, H - size + 1, (B, ))
    top_left_x = torch.randint(0, W - size + 1, (B, ))

    for i in range(B):
        color = torch.randint(0, 256, (C, ), dtype=x.dtype)
        x[i, :, top_left_y[i]:top_left_y[i] + size,
          top_left_x[i]:top_left_x[i] + size] = color[:, None, None]
...
def impulse(x, prob=0.5, impulse_value=1.):
    """Adds the color analogue of salt-and-pepper noise"""
    mask = torch.rand_like(x) < prob
    x[mask] = impulse_value
    return x
